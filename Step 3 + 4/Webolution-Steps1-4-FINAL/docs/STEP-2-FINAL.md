...
async function scrapeImpressum(lead: Lead): Promise<ImpressumResult | null> {
  if (!lead.actualUrl) return null;
  
  // STEP 1: Find Impressum link on homepage
  // We need to re-fetch the homepage HTML (or use cached from Freshness Check)
...
function createEnrichmentPlanAndRouting(
  ...
  if ((lead.estimatedPageCount ?? 0) <= 2) crawlStrategy = 'SKIP';
  ...
  const avgDealValue = 
    lead.estimatedRevenue === 'HIGH' ? 249000 :  // €2.490 (Professional)
    lead.estimatedRevenue === 'MEDIUM' ? 149000 : // €1.490
    99000;                                         // €990 (Starter)
  
  const estimatedPipelineCost = 
...
crawlStrategy = SKIP                    → Step 4 NOT dispatched (use Step 1 homepage data)
